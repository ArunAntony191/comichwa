...
export async function getMangaChapters(mangaId, limit = 500, maxTotal = 2500) {
  ...
  try {
    do {
      const url = `${BASE}/manga/${mangaId}/feed?translatedLanguage[]=en&limit=${limit}&offset=${offset}&order[chapter]=desc&includes[]=scanlation_group&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&contentRating[]=pornographic`;
      const res = await fetch(url, { next: { revalidate: 300 } });
      if (!res.ok) break;
      const data = await res.json();
...
export async function getAlternateMangaChapters(mangaTitle, englishTitle, excludeId, currentCount = 0) {
  ...
  try {
    const searchQueries = [query, `${query} Colored`, `${query} (Official Colored)`];
    let allMatches = [];
    ...
    for (const q of searchQueries) {
      ...
      if (res.ok) {
        const data = await res.json();
        const matches = (data.data || []).filter(m => m.id !== excludeId);
        allMatches.push(...matches);
      }
    }

    // Deduplicate matches by manga ID
    ...
    for (const m of matches) {
      const altTitle = m.attributes.title?.en || Object.values(m.attributes.title || {})[0] || "Alternate Version";
      
      // Skip obvious fan doujinshis unless no official alternate exists
      if (altTitle.toLowerCase().includes("doujinshi") && bestAltChapters.length > 0) {
        continue;
      }

      const { chapters: altChapters } = await getMangaChapters(m.id);
